...
if args.phase == 'train':
    ...
    CSV_FILES_NAMES_TRAIN = ['OTUS_Image_Classification_F1_Lithology_WC.csv', 'OTUS_Image_Classification_F2_Lithology_WC.csv','OTUS_Image_Classification_F3_Lithology_WC.csv']
    CSV_FILES_NAMES_TEST  = ['OTUS_Image_Classification_F1_Lithology_WC.csv', 'OTUS_Image_Classification_F2_Lithology_WC.csv','OTUS_Image_Classification_F3_Lithology_WC.csv']
    ...
    if args.continue_training:
        continue_, b, c = Recover_hyperparameters_GM(args.tracking_files_path + "General_tracking_LTWC.txt", b, c)
    while b < len(BACKBONE_NAME):
        ...
        while c < len(CSV_FILES_NAMES_TRAIN):
            ...
            Schedule.append("python " + Train_MAIN_COMMAND + " --train_task Image_Classification --learning_model CNN --backbone_name " + backbone_name + " --pretrained_backbone False --labels_type onehot_labels "
                            "--weights_definition automatic --learning_ratedecay True --lr 0.0001 --batch_size 5 --epochs 100 --patience 10 --runs 1 --phase train --tracking_training " + str(args.tracking_training) + " --continue_training " + str(continue_training) + " --identifier Lithology_WC --optimizer Adam --feature_representation True --layer_index " + layer_position + " "
...
for i in range(len(Schedule)):
    t = open(args.tracking_files_path + "General_tracking_LTWC.txt", "a")
